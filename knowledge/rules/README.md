...
| `usage_refinable` | `false` | The rule's `usage` is a DEFAULT inferred from where it matched, not stated by the call: `Signature.getInstance` names the algorithm, not the direction. A use-site annotation (`annotates: usage`) in the same function may replace it; with none, the default stands. A usage the call itself states is never replaced (ADR-0037). |
